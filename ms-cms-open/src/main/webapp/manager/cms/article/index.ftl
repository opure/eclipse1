...
<body>
	...
	<div class="container-fluid link-style">
		<div class="row">
			<div class="col-md-2 categoryTree" style="overflow-x: scroll;">
			...
			<div class="col-md-10" style="margin-top:0;padding:0;margin:0">
				...
				<div class="row" style="margin-top:0; width:100%;;padding:0 0 0 10px;margin:0">
					<iframe src="${base}/manager/cms/article/0/list.do" style="width:100%;maring:0;padding:0;border:none;height:100%" id="listFrame" target="listFrame" ></iframe>
				</div>
				<!--内容部分结束-->
			...
			var setting = {
				callback: {
					onClick: function(event, treeId, treeNode) {
						if (treeNode.type==1) {
								$("#listFrame").attr("src","${base}/manager/cms/article/"+treeNode.id+"/list.do?categoryTitle="+encodeURIComponent(treeNode.name));
						} else {
								$("#listFrame").attr("src","${base}/manager/cms/article/"+treeNode.id+"/edit.do?categoryId="+treeNode.id+"&categoryTitle="+encodeURIComponent(treeNode.name));
						}
					}
				},
				view: {
