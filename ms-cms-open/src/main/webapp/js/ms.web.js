...
(function($) {
	 ...
	 *调用该方法的元素必须存在data-ajax-url参数；
	 *参数：data-ajax-url必须 data-ajax-data 可选
	 */
	$.fn.request = function(config) {
		var target = $(this);
		if (isEmpty(target.attr("data-ajax-url")) &&isEmpty(config.url)) {
		...
		var data_type = "json";
		var func = null;
		var _url = isEmpty(target.attr("data-ajax-url"))?null:target.attr("data-ajax-url");//请求地址
		var _data = isEmpty(target.attr("data-ajax-data"))?null:target.attr("data-ajax-data");//请求参数
		var _loadingText =  isEmpty(target.attr("data-ajax-loading-text"))?null:target.attr("data-ajax-loading-text");//加载状态; 
		var data_type =  isEmpty(target.attr("data-ajax-type"))?null:target.attr("data-ajax-type");//返回数据类型
		var text = target.text();
		if(config!=undefined) {
			//请求方法
			 if (config.method!=undefined ) {
				 var _method = config.method;
				 if (_method.toLowerCase()!="post" || _method.toLowerCase()!="get") {
					 method=_method;
				 }
			} 
			 //回调方法
			 ...
			 if (config.type!=undefined){
				 ...
				 if(_type=="xml" || _type=="html" ||  _type=="script" ||  _type=="jsonp" ||  _type=="json" ||  _type=="text") {
					 data_type = _type;
				 } 
	...
	function isEmpty(target,message) {
		if (target==undefined || target ==null|| target.trim()=="" || target.trim().length==0) {
	...
	"initModal" : function() { // 初始化模态框
		...
		if ($("*[data-toggle='modal']").size() > 0) {
			$("*[data-toggle='modal']").each(function(index) {
				$("body").on("tap", "[data-target=\"" + $(this).attr("data-target") + "\"]", function() {
					if ($(this).attr("data-target") != "") {
						openModal($(this).attr("data-target"), w, h);
					}
				})
			});
		}
		...
		function openModal(modalId, w, h) {
			...
			if (!$(modalId).parent().hasClass("modalMask")) {
				$(modalId).wrap("<div class='modalMask' style='width:" + w + "px;height:" + h + "px;position: absolute;background:rgba(0, 0, 0, 0.6) none repeat scroll 0 0 !important;filter:Alpha(opacity=80); background:#fff;z-index: 9997;top: 0;'>");
			} else {
				$(modalId).parent().show();
			}
