...
public class ArticleAction extends BaseAction {
	...
	public String queryList(HttpServletRequest request, ModelMap mode, HttpServletResponse response, @PathVariable int categoryId) {
		...
		int count = articleBiz.count(websiteId, categoryId, articleType, null);
	...
	public String add(ModelMap mode, HttpServletRequest request) {
		...
		request.setAttribute("isEditCategory", false); //新增状态
		request.setAttribute("categoryId", categoryId);
		if(!categoryTitle.equals("0")){
	...
	public void save(@ModelAttribute ArticleEntity article, HttpServletRequest request, HttpServletResponse response) {
		// 获取站点id
		int webId = getManagerBySession(request).getBasicId();
		// 验证文章，文章自由排序，栏目id
		...
		if (fromTest(article, response)) {
			...
			if(column.getColumnType()==ColumnEntity.COLUMN_TYPE_LIST) {//列表
				article.setArticleUrl(column.getColumnPath()+File.separator+article.getBasicId()+".html");
			} else if (column.getColumnType()==ColumnEntity.COLUMN_TYPE_COVER) {//单篇
				article.setArticleUrl(column.getColumnPath()+File.separator+RegexConstant.HTML_INDEX);
			}
			...
			if (article.getColumn().getColumnType()==ColumnEntity.COLUMN_TYPE_COVER) {
	...
	/**
	 * 获取article_list.ftl表单信息进行修改
	 * 
	 * @param article
	 *            文章对象
	 */
	@RequestMapping("/{columnId}/update")
	public void update(@PathVariable int columnId, @ModelAttribute ArticleEntity article, HttpServletRequest request, HttpServletResponse response) {
		// 获取站点id
		int webId = getManagerBySession(request).getBasicId();
		article.setBasicUpdateTime(new Timestamp(System.currentTimeMillis()));
		// 文章类型
		...
		// 获取更改前的文章实体
		ArticleEntity oldArticle = (ArticleEntity) articleBiz.getEntity(article.getBasicId());
		// 获取栏目实体
		ColumnEntity column = (ColumnEntity) columnBiz.getEntity(article.getBasicCategoryId());
	...
	public String edit(@PathVariable int id, ModelMap model, HttpServletRequest request) {
		...
		if (_categoryId > 0) { //分类获取文章
			articleEntity =  articleBiz.getByCategoryId(_categoryId);
			model.addAttribute("article", articleEntity);
			// 文章属性
			model.addAttribute("articleType", articleType());
			
			model.addAttribute("categoryTitle",categoryTitle);
			model.addAttribute("categoryId", _categoryId);//编辑封面
	...
	public void delete(@PathVariable int id, HttpServletRequest request, HttpServletResponse response) {
		...
		// 若能找到该文章，并且在该站点下
		if (!StringUtil.isBlank(articleEntity) && articleEntity.getArticleWebId() == webId) {
			articleBiz.deleteEntity(id);
			articleBiz.deleteBasic(id);
		}
		ColumnEntity column = (ColumnEntity) columnBiz.getEntity(articleEntity.getBasicCategoryId());
		// 删除对应的新增字段的信息
		if (column.getColumnContentModelId() != 0) {
			// 获取旧的内容模型id
			ContentModelEntity contentModel = (ContentModelEntity) contentBiz.getEntity(column.getColumnContentModelId());
			// 删除旧的内容模型中保存的值
			Map wheres = new HashMap();
			wheres.put("basicId", articleEntity.getBasicId());
			if (contentModel != null) {
				fieldBiz.deleteBySQL(contentModel.getCmTableName(), wheres);
			}
		}
		String pageNo = this.getCookie(request, CookieConst.PAGENO_COOKIE);
		this.outJson(response, ModelCode.CMS_ARTICLE, true,column.getCategoryId()+"",this.redirectBack(request,false));
	}
	...
	public void allDelete(HttpServletRequest request, HttpServletResponse response) {
		...
		if (!StringUtil.isBlank(checkboxData)) {
			for (int i = 0; i < checkboxData.length; i++) {
				int number = Integer.parseInt(checkboxData[i]);
				// 获取id，查询该文章是否在该站点下
				ArticleEntity articleEntity = (ArticleEntity) articleBiz.getEntity(number);
				if (!StringUtil.isBlank(articleEntity) && articleEntity.getArticleWebId() == webId) {
					articleBiz.deleteEntity(number);
				}
			}
		}
	...
	public void queryArticleJson(HttpServletRequest request, HttpServletResponse response) {
		...
		int count = articleBiz.count(appId, _categoryId, flag, noFlag);
