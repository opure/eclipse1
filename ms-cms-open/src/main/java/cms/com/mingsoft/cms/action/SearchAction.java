...
public class SearchAction extends BaseAction {
	...
	@ResponseBody
	public ModelMap queryFieldName(@PathVariable int columnId,ModelMap model, HttpServletRequest request){
		// 获取栏目信息
	...
	public String generateSreachFormHtml(ModelMap model, HttpServletRequest request){
		...
		for(Entry<String, String[]> entry : field.entrySet()){
			...
			if(key.equals("basicCategoryId") && !StringUtil.isBlank(value) && !key.equals("searchId")){
				basicCategoryId = Integer.valueOf(value);
			}
			if(!key.equals("basicCategoryId") && !key.equals("searchId")){ 
	...
	public String list(ModelMap model, HttpServletRequest request){
		...
		int recordCount = searchBiz.queryCount();
		PageUtil page = new PageUtil(StringUtil.string2Int(pageNo),recordCount, getUrl(request) + PAGE_URL);
		List<BaseEntity> searchList = searchBiz.query(this.getAppId(request), page);
