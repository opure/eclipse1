...
public class SearchAction extends BaseAction {
	...
	private final static String PRODUCT_FIELD_PRICE="p.product_price";
	...
	public void search(HttpServletRequest request, @PathVariable int websiteId, @PathVariable int searchId, HttpServletResponse response) {
		...
		if (search != null) {
			...
			for (Entry<String, String[]> entry : field.entrySet()) {
				if (entry != null) {
					...
					if (!StringUtil.isBlank(articleField.get(entry.getKey())) && !StringUtil.isBlank(value)) {
						articleFieldName.put(entry.getKey().substring(0, 1) + "." + entry.getKey(), value);
					}else {
						if (!StringUtil.isBlank(value)) {
			...
			if (StringUtil.isInteger(basicCategoryId)) {
				...
				if (categoryIds!=null && categoryIds.length>0) {
					for(Integer id:categoryIds) {
						ids.addAll(categoryBiz.queryChildrenCategory(id));
					}
			...
			int count = 0;
			// 获取符合条件的文章总数
			count = articleBiz.getSearchCount(contentModel,whereMap, websiteId,ids);
			...
			// 列表每页显示的数量
			if (StringUtil.string2Int(property.get(RegexConstant.LIST_SIZE)) > 0) {
				size = StringUtil.string2Int(property.get(RegexConstant.LIST_SIZE));
			}
			// 页面对象
			...
			if (isMobileDevice(request) && !StringUtil.isBlank(app.getAppMobileStyle())) {
				// 对模板内容进行解析
				htmlContent = generaterFactory.buildSearch(app, htmlContent, webSiteTmpPath, no, articleList, column, page,app.getAppMobileStyle());				
			} else {
				// 对模板内容进行解析
				htmlContent = generaterFactory.buildSearch(app, htmlContent, webSiteTmpPath, no, articleList, column, page);				
			}
 ...
 */
	...
	private Map<String, List> searchMap(Map<String, Object> articleField, Map<String, String> diyFieldName, List fields) {
		...
		for (Iterator iter = articleField.keySet().iterator(); iter.hasNext();) {
			...
			// 是否为自定义字段
			list.add(false);
			if(key.equals(PRODUCT_FIELD_PRICE)){
